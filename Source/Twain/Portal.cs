...
namespace Twain
{
  public class Portal
  {
    ...
    public Portal(IntPtr windowHandle)
    {
      ...
      fApplicationId.Version.Language = TwLanguage.USA;
      fApplicationId.Version.Country = TwCountry.USA;
    ...
    {
      TwCallback cb = new TwCallback();
      cb.RefCon = 0;
      cb.CallBackProc = Marshal.GetFunctionPointerForDelegate(new TwainCallback(CallbackFn));

      TwRC rc = LibTwain32.DSMcallback(fApplicationId, IntPtr.Zero, TwDG.Control, TwDAT.Callback, TwMSG.RegisterCallback, ref cb);
      return (bool)(rc == TwRC.Success);
    }


    delegate TwRC TwainCallback(TwIdentity origin, TwIdentity dest, TwDG dg, TwDAT dat, TwMSG msg, IntPtr data);


    // In construction...
    private TwRC CallbackFn(TwIdentity origin, TwIdentity dest, TwDG dg, TwDAT dat, TwMSG msg, IntPtr data)
    {
      return TwRC.Success;
    }
    ...
    {
      // On Windows, TwEvent.EventPtr will point to WINMSG structure 
      TwEvent evtmsg;
      evtmsg.Message = 0;
